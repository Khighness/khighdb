...
func testLogFileWrite(t *testing.T, ioType IOType) {
	...
	tests := []struct {
		...
		{
			"normal-1", fields{lf: lf}, args{buf: []byte("khighdb")}, false,
		},
		{
...
func testLogFileReadLogEntry(t *testing.T, ioType IOType) {
	...
	entries := []*LogEntry{
		...
		{Key: nil, Value: []byte("khighdb"), ExpiredAt: 99400542343},
		{Key: []byte("k2"), Value: []byte("khighdb"), ExpiredAt: 8847333912},
	...
	tests := []struct {
		...
		{
			"read-entry-4", fields{lf: lf}, args{offset: offsets[4]}, &LogEntry{Key: []byte{}, Value: []byte("khighdb"), ExpiredAt: 99400542343}, int64(len(vals[4])), false,
		},
		{
