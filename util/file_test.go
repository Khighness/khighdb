...
func TestPathExist(t *testing.T) {
	path, err := filepath.Abs(filepath.Join("/tmp", "path", "khighdb-1"))
	assert.Nil(t, err)
	path2, err := filepath.Abs(filepath.Join("/tmp", "path", "khighdb-2"))
	assert.Nil(t, err)
	...
	existedFile, err := filepath.Abs(filepath.Join("/tmp", "path", "khighdb-file1"))
	assert.Nil(t, err)
	noExistedFile, err := filepath.Abs(filepath.Join("/tmp", "path", "khighdb-file2"))
	assert.Nil(t, err)
...
func TestCopyFile(t *testing.T) {
	path := filepath.Join("/tmp", "path", "khighdb-1")
	err := os.MkdirAll(path, os.ModePerm)
	assert.Nil(t, err)
	defer func() {
